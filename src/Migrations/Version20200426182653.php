 ...
 */
final class Version20200426182653 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__friend AS SELECT id, user_id, friend_id, status FROM friend');
        $this->addSql('DROP TABLE friend');
        $this->addSql('CREATE TABLE friend (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, friend_id INTEGER NOT NULL, status VARCHAR(255) NOT NULL COLLATE BINARY, CONSTRAINT FK_55EEAC61A76ED395 FOREIGN KEY (user_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_55EEAC616A5458E8 FOREIGN KEY (friend_id) REFERENCES user (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO friend (id, user_id, friend_id, status) SELECT id, user_id, friend_id, status FROM __temp__friend');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__friend AS SELECT id, user_id, friend_id, status FROM friend');
        $this->addSql('DROP TABLE friend');
        $this->addSql('CREATE TABLE friend (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, user_id INTEGER NOT NULL, friend_id INTEGER NOT NULL, status VARCHAR(255) NOT NULL)');
        $this->addSql('INSERT INTO friend (id, user_id, friend_id, status) SELECT id, user_id, friend_id, status FROM __temp__friend');
        $this->addSql('DROP TABLE __temp__friend');
        $this->addSql('CREATE INDEX IDX_55EEAC61A76ED395 ON friend (user_id)');
        $this->addSql('CREATE INDEX IDX_55EEAC616A5458E8 ON friend (friend_id)');
